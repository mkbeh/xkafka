...
func establishTestShareGroup(t *testing.T, client *Client) {
	...
	if !errors.Is(err, context.DeadlineExceeded) {
		t.Fatalf("establish share group: %v, want %v", err, context.DeadlineExceeded)
	}
}

func waitTestCommittedOffset(
	t *testing.T,
	cluster *kfake.Cluster,
	group string,
	topic string,
	...
	if err != nil {
		t.Fatalf("create offset fetch client: %v", err)
	}
	defer client.Close()
	...
	for time.Now().Before(deadline) {
		...
		offsets, err := admin.FetchOffsets(kadm.RequireStable(ctx), group)
		cancel()
		if err != nil {
			lastErr = err
	...
	if lastErr != nil {
		t.Fatalf("timed out waiting for committed offset %d: last error: %v", want, lastErr)
	}
	t.Fatalf("timed out waiting for committed offset %d: last offset = %d", want, lastOffset)
}
